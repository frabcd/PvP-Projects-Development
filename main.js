...
import { loadEmperorAssets } from './assets.js';
import {
...
// NEW: Load assets before starting
async function init() {
    try {
        await loadEmperorAssets((progress) => {
            const percent = Math.round(progress * 100);
            $$('loading-bar-fill').style.width = `${percent}%`;
            $$('loading-status').textContent = `Loading Emperor Assets: ${percent}%`;
        });
        ...
        animate(); // Start the loop after assets are ready
    } catch (err) {
        console.error("Critical error loading assets:", err);
        $$('loading-status').textContent = "Failed to load assets. Please refresh.";
    }
}
