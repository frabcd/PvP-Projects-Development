...
import { emperorAssets } from './assets.js';
import {
...
export class Player {
    ...
    constructor(playerNum, characterKey, aiTier = null) {
        ...
        // --- NEW: Animation Setup for Emperor ---
        this.mixer = null;
        this.actions = {};
        this.currentAction = null;
        this.skillAnimationTimer = 0;

        if (this.characterKey === 'MIRAGE' && emperorAssets.model) {
            this.mixer = new THREE.AnimationMixer(this.mesh);
            Object.keys(emperorAssets.animations).forEach(key => {
                const clip = emperorAssets.animations[key];
    ...
    fadeToAction(name, duration = 0.2, loop = true) {
    ...
    update(delta, opponent, specialObjects) { // <-- BUG-FIX: Added specialObjects
        ...
        if (this.skillAnimationTimer > 0) this.skillAnimationTimer -= delta;
        ...
        // --- NEW: Emperor Animation Logic ---
        if (this.mixer && this.characterKey === 'MIRAGE' && this.skillAnimationTimer <= 0) {
            if (this.velocity.lengthSq() > 0.5) {
                // Calculate local velocity relative to mesh orientation
                // Since lookAt makes +Z point at target, we need to know if we are moving
                // towards target (+Z), away (-Z), or sideways.
                const localVel = this.velocity.clone().applyQuaternion(this.mesh.quaternion.clone().invert());

                // Mixamo / FBX forward is usually +Z. 
                // But Three.js lookAt makes +Z point at target.
                // So if localVel.z > 0, we are moving towards target.

                if (Math.abs(localVel.x) > Math.abs(localVel.z)) {
                    if (localVel.x > 0) this.fadeToAction('walk_r');
                    else this.fadeToAction('walk_l');
                } else {
                    // Reversed Z because lookAt points +Z at target, 
                    // but the model might be facing -Z or +Z.
                    // Usually models face +Z in their own local space.
                    // If moving towards target (localVel.z > 0), play walk_f
                    if (localVel.z > 0) this.fadeToAction('walk_f');
                    else this.fadeToAction('walk_b'); // User said use walk_r for back, but walk_b is aliased to walk_r
                }
            } else {
    ...
    executeSkill(skillKey, isRemote = false) {
        ...
        // --- NEW: Emperor Skill Animations (Moved to top) ---
        if (this.mixer && this.characterKey === 'MIRAGE') {
            if (skillKey === 'basicAttack') {
                this.fadeToAction('attack', 0.1, false);
                this.skillAnimationTimer = 0.5;
            } else if (skillKey === 's1') {
                this.fadeToAction('s1', 0.1, false);
                this.skillAnimationTimer = 0.8;
            } else if (skillKey === 's3') {
                this.fadeToAction('s3', 0.1, false);
                this.skillAnimationTimer = 0.6;
            } else if (skillKey === 's4') {
                this.fadeToAction('s4', 0.1, false);
                this.skillAnimationTimer = 1.5;
            }
        }
